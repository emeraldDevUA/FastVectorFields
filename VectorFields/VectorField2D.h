...
template <typename T>
class VectorField2D : public AbstractField2D<vfMath::Vector2D<T>>
{
public:
    explicit VectorField2D(size_t grid_size)
        : AbstractField2D<vfMath::Vector2D<T>>(grid_size)
    {}

    VectorField2D(size_t x_size, size_t y_size)
        : AbstractField2D<vfMath::Vector2D<T>>(x_size, y_size)
    {}

    explicit VectorField2D(const ScalarField2D<T>& field)
        : AbstractField2D<vfMath::Vector2D<T>>(field.x_size, field.y_size)
    {
